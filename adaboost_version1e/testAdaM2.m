...
for num_ite = 1:2
    all_acc = [];
    for j=1:6
        clearvars -except j file_name all_acc all_acc_ite num_ite;
        t = cputime;
        load(file_name{j});
        ...
        % [info2,data2,meta2]=transformIDM_selectROIVoxels(info1,data1,meta1,{'CALC' 'LIPL' 'LT' 'LTRIA' 'LOPER' 'LIPS' 'LDLPFC'}); %7
        % [info2,data2,meta2]=transformIDM_selectROIVoxels(info1,data1,meta1,{'CALC' 'LIPL' 'LIPS' 'LOPER'}); %4 theo bai bao
        %[info2,data2,meta2]=transformIDM_selectROIVoxels(info1,data1,meta1,{'CALC' 'LIPL' 'LT' 'LTRIA' }); %4 co ket qua tot nhat theo a Ngoc
        [info2,data2,meta2]=transformIDM_selectROIVoxels(info1,data1,meta1,{'CALC'}); %1 theo ca bai bao va a Ngoc

        % lay theo negative va positive
        % [infoP1,dataP1,metaP1]=transformIDM_selectTrials(info1,data1,meta1,find([info1.cond]==2));
        % [infoS1,dataS1,metaS1]=transformIDM_selectTrials(info1,data1,meta1,find([info1.cond]==3));
        % [infoF1,dataF1,metaF1]=transformIDM_selectTrials(info1,data1,meta1,find([info1.cond]==1));
        ...
        % colss = randi(size(examples, 2), 1, 100);
        % examplesPS = examplesP(:,colss); 
        % examplesSS = examplesS(:,colss);
    ...
    disp('------------------------------');
    all_acc_ite = [all_acc_ite; mean_all(1) mean_all(2)];
end

% mean_all = mean(all_acc_ite);

% disp(['TOTAL AVERAGE ACC', num2str(mean_all(1))]);
% disp(['TOTAL AVERAGE PROCESSING TIME ', num2str(mean_all(2))]);