...
num_nodes = [7 8 10 14 16 20 32 40];
...
N1=160; % Approximately 20 for each one of the four clusters
s=4;
for i=1:c1
    S1(:,:,i)=s*eye(l);
end
...
pe_all = [];
for itt_num_nodes=1:length(num_nodes)
	% 2. 
	k=num_nodes(itt_num_nodes); %number of hidden layer nodes. Also use 4 and 10 nodes
	...
	pe_all = [pe_all; pe_train pe_test];
end
